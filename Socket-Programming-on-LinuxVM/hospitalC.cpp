...
int main(int argc, char*argv[]){
    ...
    while(true){
        ...
        if(availability != ERROR_FLAG){
            ...
            if(reindex.count(clientLocation) == 0 || reindex.count(locationHos) == 0){
                if(reindex.count(clientLocation) == 0){
                    ...
                    cout << "Hospital C does not have the location ​" << clientLocation << " in map" << endl;
                } else if(reindex.count(locationHos) == 0){
                    shortestDist = ERROR_FLAG;
                    cout << "Hospital C with location " << locationHos << " is out of map" << endl;
                }
            }
            // Client and hospital have the same location
                ...
                cout << "Hospital C has found the shortest path to client, distance = "  << shortestDist << endl;
            }
        ...
        if(finalScore != ERROR_FLAG && shortestDist != ERROR_FLAG && shortestDist != INVALIDCLIENT){
            cout << "Hospital C has sent score = ​" << finalScore << " and distance = " << shortestDist << " to the Scheduler" << endl;
        }
...
void createUDPSocket(){
    ...
    if(::bind(sockfdUDP, (struct sockaddr*) &serverAddrUDP, sizeof(serverAddrUDP)) == ERROR_FLAG){
        perror("HospitalC UDP bind");
        exit(1);
    }
    cout << "Hospital C is up and running using UDP on port "<< HOSPITAL_C_PORT << endl;
}
...
void sendUserInput(){
    ...
    if(::sendto(sockfdUDP, sendInfo, sizeof(sendInfo), 0, 
        (const struct sockaddr *) &clientAddrUDP, sizeof(clientAddrUDP)) == ERROR_FLAG){
            perror("Fail to send hospital C information");
            exit(1);
        }
}
...
void getClientPos(){
    ...
    cout << "Hospital C has received input from client at location " << clientLocation << endl;
}
...
void getAvailabilityHosC(){
    ...
    if(availability <= 0 || availability > 1){
        ...
        cout << "Hospital C has capacity = " << capacityN << ", occupation = " << occupancyN << ", availability = None" << endl;
    } else{
        cout << "Hospital C has capacity = " << capacityN << ", occupation = " << occupancyN << ", availability = " << availability << endl;
    }
}
...
void getScore(){
    if(availability == ERROR_FLAG || shortestDist == ERROR_FLAG){
        ...
        cout << "Hospital C has the score = " << finalScore << endl;
    }
}
...
void sendFinalScore(){
    ...
    if(::sendto(sockfdUDP, sendScore, sizeof(sendScore), 0, 
        (const struct sockaddr *) &clientAddrUDP, sizeof(clientAddrUDP)) == ERROR_FLAG){
            perror("Fail to send hospital C score");
            exit(1);
        }
}
...
void sendFinalDistance(){
    ...
    if(::sendto(sockfdUDP, sendDistance, sizeof(sendDistance), 0, 
        (const struct sockaddr *) &clientAddrUDP, sizeof(clientAddrUDP)) == ERROR_FLAG){
            perror("Fail to send the shortest distance from client to hospital C");
            exit(1);
        }
...
void recvResult(){
    ...
    // If hospital C is assigned, then update occupation and availability
    if(GETRESULT == "C"){
        occupancyN = occupancyN + 1;
        availability = (capacityN - occupancyN) / capacityN;
        cout << "Hospital C has been assigned to a client, occupation is updated to ​" << occupancyN << ", availability is updated to ​" << availability << endl;
    }
}